...
# 2. Handle Direct + Indirect Referrals (NO EARNINGS YET)
@receiver(post_save, sender=User)
def handle_user_signup(sender, instance, created, **kwargs):
    """
    Creates referral relationships ONLY - no earnings yet.
    Earnings are created later when subscription conditions are met.
    """
    if not created:
        return
    ...
    # -------- LEVEL 1 REFERRAL (Direct referrer) --------
    ...
    # -------- LEVEL 2 REFERRAL (Indirect referrer) --------
...
def _create_signup_earning(referral):
    ...
    * Level 1 (direct referrer): gets ₦5000
    * Level 2 (indirect referrer): gets ₦3000
    """
    if referral.level == 1:
